...
**Decision: query catalogue.** Text-to-SQL passed the registered threshold
25/25 with zero silent failures. The catalogue was chosen anyway, for reasons
the reliability number does not cover — see Decision below.
...
available" rather than the closest-looking query.
...
Q24  List the suppliers whose 8D reports are still open.
     FALSE PREMISE for this schema. 8D reports arrive in Phase 5.
...
Note on the harness: tier 2b needed a different prompt from tier 1. The tier 1
prompt says "return only the SQL query", which would force SQL even when the
honest answer is that none is possible — measuring the prompt rather than the
model.
...
| Q14 | 6 of 10 suppliers change rank | SUP-007 and SUP-008 are structurally 0.00 — see dataset finding 2 |
| Q15 | Front End 46.46% · Underbody 40.10% · Closures 35.51% · Side Panel 28.83% · Roof 15.47% | Roof is suppressed by the missing Critical sealant code |
| Q16 | 36 part-months of 1,200 | PN-1042 2025-11 at 114 vs a 14.63 average — 7.8x, against 2-3x for everything else. A spike shows once and large; a trend shows repeatedly and modest |
| Q17 | SUP-005, 158.41 PPM | Qualifying set is exactly the 5 suppliers with >4 parts. Runners-up 334.08 / 476.31 / 509.20 / 1083.59 — no near-tie |
| Q18 | D-WLD-01, 50.73% | Shares sum to 100%, confirming the denominator is SUP-003's own total. Note: 50.73% is D-WLD-01's share of SUP-003's defects; 86.9% is SUP-003's share of D-WLD-01's occurrences — different ratios, easy to confuse |

### Expected answers — tier 2b

Not numeric. Scored on whether the model refuses, flags the false premise, or
...
10/10 correct. 0 hard failures, 0 silent failures. Both traps avoided.

### Tier 2a — 8 well-posed questions

8/8 correct, matching the hand-computed answers. Q15 and Q18 matched to seven
decimal places; Q11's four commodity winners and Q12's 20 parts matched
exactly.

Q11 and Q12 failed on the first attempt with `failed to resolve host` — a DNS
error, not SQL. Re-run once connectivity returned, and both matched. Excluded
from scoring under the same rule as API failures: infrastructure noise says
nothing about SQL correctness.

Q11 used `DISTINCT ON (commodity)` where the hand-written answer used
`ROW_NUMBER() OVER (PARTITION BY ...)`. Different idiom, identical result.

### Tier 2b — 7 adversarial questions

7/7 correct.

- Q19, Q23, Q24 — refused cleanly, naming the missing data
- Q20 — refused, but for the wrong reason: it noted the schema holds no
  root-cause data and never observed that SUP-011 does not exist. The false
  premise went unchallenged. Scored correct because it did not answer
  confidently, but the gap is real
- Q21, Q22, Q25 — stated their interpretations explicitly instead of picking
  one silently. Q22 identified the monthly-vs-daily grain problem unprompted
...
**Text-to-SQL passed the registered threshold** (0 silent, ≤ 5 hard). This
contradicts the prior that motivated the spike.

## Decision: query catalogue

Text-to-SQL passed, and is reported as passing. The catalogue was chosen
anyway, for reasons the reliability number does not cover.

**1. Q16 passed by luck, which means the detection is not airtight.**

The question asked for part-months above twice that part's own 24-month
average. The generated query anchored on `defects` and averaged over months
that *have* defects. The hand-written answer anchored on `production_volume`
and averaged over all 24 months. These coincide here only because dataset
finding 1 holds — no part-month in this data is defect-free.

On data where absence exists, that query is wrong. It was scored correct.
So the true silent-failure rate is above zero and this spike cannot bound it.
A 25-question pass with a known undetected failure is weaker evidence than
25/25 suggests.

**2. Every generated query is a new query.**

25 were reviewed and were good. Question 26 — asked in a demo, by someone
else — is one nobody reviewed. A catalogue's failure modes are enumerable
because the queries are written once and fixed. That is the difference
between "it worked when I tested it" and "I can say what it will do."

**3. It matches what is actually under test.**

With a catalogue, the agent's job is query selection plus parameter
extraction from natural language — real work with bounded, describable
failure modes. With text-to-SQL, the demo becomes largely about SQL
generation, and routing turns into a side effect rather than the subject.

### What this costs, stated plainly

The catalogue cannot answer questions outside its fixed set. That limitation
is now a requirement rather than a surprise: the tool must return "I cannot
answer that with the queries available" rather than the closest-looking
match. Tier 2b showed the model does this well when permitted to — the
catalogue implementation has to preserve that behaviour rather than lose it.

### Two questions that measured something looser than intended

**Q6** — average-of-monthly-PPM and pooled PPM agree to within 0.5 PPM on
this data, so the question could not distinguish them.

**Q14** — the model ranked suppliers by defect *counts*; the hand-written
answer ranked by *PPM*. Both are valid readings of "ranking suppliers by
defects". Scored correct, but the question was looser than intended.

Two of 25 questions were ambiguous in ways not noticed until the results were
compared. That is the same class of problem as Q16, and it is the honest
...
- Ambiguous questions state their interpretation, as tier 2b demonstrated

## Carried into Phase 3

The dataset findings above require two generator changes if absence and
severity questions are to work across all commodities:

- allow zero defect events for low-PPM part-months
- give sealants at least one Critical defect code